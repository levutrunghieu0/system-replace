...
export const Route = createFileRoute('/inventory/price-issue')({
  component: PriceIssuePage,
})
...
function PriceIssuePage() {
  ...
  const handleOpenEdit = (item: PriceItem) => {
    ...
    setEditMin(item.originalCount)
  }

  const handleEditConfirm = () => {
    if (!editId) return
    setItems(prev => prev.map(i => i.id === editId ? { ...i, count: editCount } : i))
    setEditId(null)
    setTimeout(() => inputRef.current?.focus(), 0)
  }
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1, height: '100%' }}>
          ...
          <TableBody>
            ...
            {items.map((item) => (
              <TableRow key={item.id} hover>
                ...
                <TableCell sx={{ fontSize: '0.82rem', textAlign: 'center', whiteSpace: 'nowrap' }}>
                ...
                <TableCell sx={{ textAlign: 'center', py: 0.25, whiteSpace: 'nowrap' }}>
                  <IconButton
                    size="small"
      ...
      <AppModal
        open={editId !== null}
        title={t('page.priceIssue.editModal.title')}
      ...
      >
        <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 1.5, py: 1 }}>
          <Typography sx={{ fontSize: '0.825rem', color: 'text.secondary' }}>
            {t('page.priceIssue.editModal.label')}
          </Typography>
          <QuantityStepper value={editCount} onChange={setEditCount} min={editMin} />
        </Box>
      </AppModal>
